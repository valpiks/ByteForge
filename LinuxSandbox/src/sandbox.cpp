...
private:
	void run_program_with_pipes()
    {
        ...
        while (true)
        {
            ...
            if (elapsed_ms > time_limit_sec * 1000 && !memory_exceeded)
            {
            ...
            if (memory_exceeded)
            {
                log("Memory limit exceeded, process killed");
                timed_out = false;
            }
            ...
            if (w == pid)
            {
                ...
                bool was_bad_alloc = (program_output.find("std::bad_alloc") != std::string::npos) ||
                                    (program_output.find("bad_alloc") != std::string::npos);

                if (was_bad_alloc && !memory_exceeded) {
                    memory_exceeded = true;
                    log("Detected std::bad_alloc - memory limit exceeded");
                }
            ...
            {
                program_finished = true;
                log("Process terminated due to limits");
                waitpid(pid, NULL, 0);
                close(fd_stdout);
                close(fd_stdin);
                send_json_result(memory_exceeded ? -5 : -4, program_output,
                               memory_exceeded ? "Memory limit exceeded" : "Time limit exceeded",
